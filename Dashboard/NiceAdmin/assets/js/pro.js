window.addEventListener('DOMContentLoaded', function () {
    ...
    // Khởi tạo modal thêm sản phẩm nếu có
    ...
    if (addProductModalElement) {
        ...
        // Nếu cần dùng addProductModal ở nơi khác, có thể lưu ra biến toàn cục
...
function renderProductTable() {
    ...
    products.forEach((product, idx) => {
        ...
        // Tạo hàng chi tiết sản phẩm (phải truyền products vào để hàm hoạt động)
        const detailRow = createProductDetailRow(product, products);
...
document.addEventListener('DOMContentLoaded', async () => {
   ...
   function renderProductTable() {
    ...
    products.forEach((p, index) => {
        ...
        checkbox.addEventListener('click', e => toggleProductSelection(e, index));
        ...
        tdStock.textContent = p.stock != null ? p.stock : '';
        tr.appendChild(tdStock);

        // Trạng thái kinh doanh
        const tdBusiness = document.createElement('td');
        if (p.businessStatus) {
            tdBusiness.innerHTML = '<span class="badge bg-success">Kinh doanh</span>';
        } else {
            tdBusiness.innerHTML = '<span class="badge bg-danger">Ngừng KD</span>';
        }
